package Combination.world.blocks.power;

import Combination.world.blocks.*;
import arc.*;
import arc.math.*;
import arc.util.*;
import arc.util.io.*;
...
public class VariableImpactReactor extends ImpactReactor{
    ...
    public void setStats(){
        ...
        stats.add(CMStat.powerTarget, Mathf.round(minPowerLevel * 60f), StatUnit.powerUnits);
        stats.add(CMStat.minLoad, minPowerFract * 100f, StatUnit.percent);
        if(maxPowerFactor > 0)
            stats.add(CMStat.maxLoad, maxPowerFactor * 100f, StatUnit.percent);
    }
